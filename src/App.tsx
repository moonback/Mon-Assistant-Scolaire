...
function AppContent() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col md:flex-row font-sans text-slate-800">
      ...
      {/* Desktop Sidebar */}
      <aside className={`hidden md:flex flex-col bg-white border-r border-slate-100 shadow-xl fixed h-screen z-40 transition-all duration-500 ease-in-out ${isSidebarCollapsed ? 'w-24' : 'w-64'}`}>
        <div className={`p-6 transition-all duration-500 ${isSidebarCollapsed ? 'px-4' : 'px-6'}`}>
          <div
            className="flex items-center gap-3 cursor-pointer group"
            onClick={() => setActiveTab('home')}
          >
            <div className="magical-gradient p-2.5 rounded-xl text-white shadow-lg shadow-indigo-200 group-hover:scale-110 transition-transform">
            ...
            {!isSidebarCollapsed && (
              ...
              >
                ...
                <p className="text-[9px] uppercase tracking-[0.2em] font-bold text-slate-400">Assistant</p>
              </motion.div>
            )}
          </div>
        </div>
        ...
        <button
          ...
          className="absolute -right-3 top-24 w-6 h-6 bg-white border border-slate-100 rounded-full shadow-md flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-all z-50 hover:bg-indigo-50"
        >
          <motion.div
        ...
        <nav className="flex-1 px-3 space-y-1.5 overflow-y-auto pb-8 scrollbar-hide">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as Tab)}
                className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all relative group ${isActive
              ...
              >
                ...
                <Icon className={`w-5 h-5 flex-shrink-0 ${isActive ? 'text-indigo-600' : 'group-hover:text-indigo-500 transition-colors'}`} />
                {!isSidebarCollapsed && <span className="font-bold text-sm whitespace-nowrap">{tab.label}</span>}
              </button>
            );
          })}
        </nav>

        <div className={`p-4 border-t border-slate-50 transition-all duration-500 ${isSidebarCollapsed ? 'items-center' : ''}`}>
          {!isSidebarCollapsed ? (
            <div className="bg-indigo-600 rounded-[2rem] p-5 mb-4 text-white relative overflow-hidden group">
              <div className="absolute -right-3 -bottom-3 opacity-10 group-hover:scale-110 transition-transform">
                <Trophy className="w-16 h-16" />
              </div>
              <div className="relative z-10">
                <p className="text-[8px] font-black uppercase tracking-widest opacity-70 mb-1.5">Prochain Rang</p>
                <h4 className="text-sm font-black mb-3">Génie Magique</h4>
                <div className="h-1.5 bg-white/20 rounded-full mb-1.5">
                  <motion.div
                    initial={{ width: 0 }}
                ...
                <p className="text-[8px] font-bold opacity-70">{profile?.stars || 0} / 1000 étoiles</p>
              </div>
            </div>
          ) : (
            <div className="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center text-white mb-4 mx-auto shadow-lg">
              <Trophy className="w-5 h-5" />
            </div>
          )}

          <div className={`bg-slate-50 rounded-xl p-3 mb-3 flex items-center transition-all ${isSidebarCollapsed ? 'justify-center' : 'gap-3'}`}>
            ...
            {!isSidebarCollapsed && (
              <div className="flex-1 min-w-0">
                <p className="font-bold text-slate-800 truncate text-xs">{profile?.username}</p>
                <div className="flex items-center gap-1">
                  <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                  <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">{profile?.grade_level}</p>
                </div>
              </div>
            )}
          </div>
          <button
            onClick={signOut}
            className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border-2 border-slate-100 text-slate-500 font-bold text-xs hover:bg-red-50 hover:text-red-500 hover:border-red-100 transition-all ${isSidebarCollapsed ? 'px-0' : ''}`}
          >
