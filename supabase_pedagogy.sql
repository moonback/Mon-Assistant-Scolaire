...
CREATE TABLE IF NOT EXISTS public.pedagogical_explanations (
  ...
  understanding_score integer CHECK (understanding_score >= 0 AND understanding_score <= 10),
...
DROP POLICY IF EXISTS "Users can manage pedagogical explanations of their children." ON public.pedagogical_explanations;
CREATE POLICY "Users can manage pedagogical explanations of their children." ON public.pedagogical_explanations
  FOR ALL USING (auth.uid() IN (SELECT parent_id FROM public.children WHERE id = child_id));
